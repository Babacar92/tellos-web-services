...
import { BUSINESS_MARKET_TYPE_PROVIDERS_NAMES } from '../dto/provider/business-market-type.providers';
import { BusinessMarketTypeLogger } from '../logger/business-market-type.logger';

@Injectable()
export class BusinessMarketTypeService extends AbstractRepositoryService {
     ...
     */
    public constructor(
        @Inject(BUSINESS_MARKET_TYPE_PROVIDERS_NAMES.DEFAULT_REPOSITORY)
        private readonly _defaultUserRepository: Repository<BusinessMarketTypeEntity>,
        private readonly _logger: BusinessMarketTypeLogger,
    ) {
        super();
    }
     ...
     */
    public async exist(
        id?: number | BusinessMarketTypeEntity,
        withDeleted?: boolean,
        repo?: string,
        manager?: EntityManager,
    ): Promise<boolean> {
        if (id instanceof BusinessMarketTypeEntity) id = id.id;
    ...
    public async existByColumn(
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (BusinessMarketTypeEntity.isColumnString(column)) {
     ...
     * @returns 
     */
    public async findBusinessMarketTypesAndPaginationAll(
        filter: BusinessMarketTypeFilterArgInput,
        sort: DatabaseSortArg,
    ...
    public async create(
        ...
        return this.useTransaction(async transaction => {
            ...
            const businessMarketType = new BusinessMarketTypeEntity();

            // Get uploaded file
            const { ...rest } = data;

            // Set Data
            Object.assign(businessMarketType, rest);

            // Save hit
            const result = await transaction.save(businessMarketType);

            if (result) {
                this._logger.create(businessMarketType);
    ...
    public async update(
        ...
        return this.useTransaction(async transaction => {
            ...
            // Find existing
            const oldBusinessMarketType = await this.findOne(id, repo, transaction);

            if (oldBusinessMarketType) {
                // Set old data
                this._logger.setOldData(oldBusinessMarketType);

                // Add new Data
                Object.assign(oldBusinessMarketType, req);

                // Save Data
                const result = await transaction.save(oldBusinessMarketType);

                if (result) {
                    this._logger.update(oldBusinessMarketType);
     ...
     */
    public async remove(
        req: BusinessMarketTypeRemoveArgInput,
        repo?: string,
        manager?: EntityManager,
    ): Promise<boolean> {
        return this.useTransaction(async transaction => {
            if (req && req.id && req.type) {
                ...
                id = id instanceof BusinessMarketTypeEntity ? id.id : id;
                const businessMarketType = await this.findOne(id, repo, transaction);

                if (businessMarketType instanceof BusinessMarketTypeEntity) {
                    if (type === REMOVE_TYPES.HARD) {
                        await transaction.delete(BusinessMarketTypeEntity, businessMarketType.id);

                        this._logger.delete(businessMarketType);
                    } else {
                        await transaction.softDelete(BusinessMarketTypeEntity, businessMarketType.id);

                        this._logger.softDelete(businessMarketType);
                    }
    ...
    private _initSelect(
        repo?: string,
        manager?: EntityManager,
    ): SelectQueryBuilder<BusinessMarketTypeEntity> {
        const qb = this.getRepo(repo)
            .createQueryBuilder('bmt', manager?.queryRunner);
    ...
    private async _applyFilter(
        qb: SelectQueryBuilder<BusinessMarketTypeEntity>,
        filter?: BusinessMarketTypeFilterArgInput,
        sort?: DatabaseSortArg,
    ): Promise<void> {
    ...
    public getRepo(repo?: string): Repository<BusinessMarketTypeEntity> {
    ...
    private _cn(columnName?: string): string | undefined {
        return BusinessMarketTypeService.ColumnQueryNames.get(columnName);
    }
