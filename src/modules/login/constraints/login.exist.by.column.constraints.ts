...
@ValidatorConstraint({
    name: 'LoginExistByColumnConstraint',
    async: true
})
@Injectable()
export class LoginExistByColumnConstraint implements ValidatorConstraintInterface {
    ...
    public defaultMessage?(validationArguments?: ValidationArguments): string {
        ...
        if (property === 'email') {
            return this._translationService.trans('The email {value} doesn\'t exist', {
                value: value,
            }, domain).getValue();
        } else if (property === 'username') {
            return this._translationService.trans('The username {value} doesn\'t exist', {
                value: value,
            }, domain).getValue();
        }
